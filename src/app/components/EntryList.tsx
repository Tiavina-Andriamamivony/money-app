      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {entries.map((entry)=>(
            ...
            <div>
              <span className={`font-semibold ${entry.type === "income" ? "text-green-500":"text-red-500"}`}>{entry.amount} Ar</span>
              <p className="text-gray-600">{entry.category}</p>
              <p className="text-sm text-gray-500">{entry.frequency}</p>

            </div>

            <button onClick={()=>{onDeleteEntry(entry.id)}} className= "border p-2 rounded-lg text-red-500 hover:text-red-600 bg-red-300 "> Supprimer</button>
